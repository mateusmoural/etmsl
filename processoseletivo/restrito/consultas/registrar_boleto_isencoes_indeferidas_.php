<?php 
  ...
  //Cria variaveis para que não sejam tratados como constantes dentro do loop  
  $id;
  $nome;
  $cpf;
  $cep;
  ...
  $query = "SELECT * FROM processoseletivo  WHERE cpf <> '' AND pagamento = 'ISENCAO INDEFERIDA'ORDER BY nome ASC ";
  ...
  while ($row = mysql_fetch_array($result)) {
    ...
    $boleto_opcao_curso = limitarTexto($opcao_curso,14);
    ...
    $situacao    = "ISENCAO INDEFERIDA - AGUARDANDO PAGAMENTO";
    $pagamento   = "PENDENTE";
    ...
    echo '<button type="button" class="btn btn-info btn-block" id="voltar" onclick="window.location.href = '."'registrar_boleto_isencoes_indeferidas_.php'".'">Próximo</button><br><br>';
